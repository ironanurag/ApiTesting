...
class JsonHighlight extends StatefulWidget {
  final List<dynamic> jsonObj;
  const JsonHighlight(this.jsonObj, {super.key});
  @override
  _JsonHighlightState createState() => _JsonHighlightState();
}

class _JsonHighlightState extends State<JsonHighlight> {
  @override
  Widget build(BuildContext context) {
    if (widget.jsonObj.isEmpty) {
      return const Center(
        child: Text('No JSON objects to display.'),
      );
    }

    return
         getContentWidget(widget.jsonObj);
  }
      ...
      list.add(Row(
        ...
        children: <Widget>[
          ...
          (ex && ink)
              ...
              onTap: () {
                setState(() {
                  openFlag[entry.key] = !(openFlag[entry.key]
                      ?? false);
                });
              })
              : Text(entry.key,
