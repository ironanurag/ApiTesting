...
class LoadTestPage extends StatefulWidget {
  @override
  _LoadTestPageState createState() => _LoadTestPageState();
}

class _LoadTestPageState extends State<LoadTestPage> with AutomaticKeepAliveClientMixin{
  @override
  ...
  int numberOfTests = 100; // Default number of tests
  ...
  Future<void> _performTest(String url, int index) async {
    try {
      ...
      if (response.statusCode == 200) {
        addLog('Test $index Success with status code ${response.statusCode}');
      } else {
        addLog('Test $index Failed with status code ${response.statusCode}');
      }
    } catch (error) {
      // Handle any errors
      addLog('Test $index Error $error');
    }
  }
